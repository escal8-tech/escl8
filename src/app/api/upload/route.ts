...
export async function POST(request: Request) {
  try {
    ...
    const businessId = auth.businessId.replace(/[^a-zA-Z0-9_-]/g, "_");
    ...
    const uploadDir = path.join(process.cwd(), "uploads", businessId);
    ...
    for (const f of files) {
      ...
      if (f.size > MAX_FILE_SIZE) {
        return NextResponse.json(
          { error: `File too large: ${f.name}. Max limit 10MB.` },
          { status: 413 }
        );
      }

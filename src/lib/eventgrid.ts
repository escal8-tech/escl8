...
export const eventGridClient: EventGridPublisherClient<any> | null = endpoint && key
  ? new EventGridPublisherClient(endpoint, 'EventGrid', new AzureKeyCredential(key))
  : null;

export async function publishEvent(eventType: string, subject: string, data: any) {
  if (!eventGridClient) {

...
export const messagesRouter = router({
  ...
  listMessages: businessProcedure
    ...
    .query(async ({ ctx, input }) => {
      return readSupport.listMessages(ctx, input);
    }),
  ...
  listThreadWindow: businessProcedure
    ...
    .query(async ({ ctx, input }) => {
      return readSupport.listThreadWindow(ctx, input);
    }),

  getThreadSessionWindow: businessProcedure
    .input(z.object({ threadId: z.string().min(1) }))
    .query(async ({ ctx, input }) => {
      return readSupport.getThreadSessionWindow(ctx, input);
    }),
  ...
  sendText: businessProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return lifecycleSupport.sendText(ctx, input);
    }),
  ...
  sendMedia: businessProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return lifecycleSupport.sendMedia(ctx, input as any); // Cast because of complex union
    }),
});

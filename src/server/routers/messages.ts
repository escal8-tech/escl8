...
export const messagesRouter = router({
  listRecentThreads: businessProcedure
    ...
    .query(async ({ ctx, input }) => {
      const cacheKey = `listRecentThreads_${ctx.businessId}_${input.limit}_${input.channelIdentityId || "all"}`;
      return withStatsCache(cacheKey, 60, () =>
        readSupport.listRecentThreads({

...
import { withStatsCache, invalidateStatsCacheByPrefix } from "../lib/statsCache";
import { delCached } from "@/lib/redis";
...
export async function assembleBusinessSetupStatus(businessId: string) {
  const cacheKey = `business:setupStatus:${businessId}`;
...
export async function setWhatsappIdentityAutoReplyPausedForBusiness(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
...
export async function setWhatsappIdentityAiDisabledForBusiness(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
  phoneNumberId: string;
  aiDisabled: boolean;
}) {
  const details = await db
...
export async function updateBookingConfigForBusiness(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
  businessId: string;
...
export async function updateTimezoneForBusiness(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
  businessId: string;
  timezone: string;
}) {
...
export async function updateOrderSettingsForBusiness(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
  businessId: string;
...
export async function updateCustomizationSettingsForBusiness(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
  businessId: string;
  businessName?: string;
  logoBlobPath?: string;
...
export async function ensureWebsiteWidgetForBusiness(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
  businessId: string;
}) {
  const [biz] = await db.select().from(businesses).where(eq(businesses.id, input.businessId)).limit(1);
  if (!biz) {
...
export async function disconnectGmailConnectionForBusiness(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
  businessId: string;
}) {
  const now = new Date();
  const [updated] = await db
...
export async function completeOnboardingSetupForBusiness(ctx: { businessId: string; userEmail?: string | null }, input: {
  businessName: string;
  website?: string;
  phone?: string;
  address?: string;
  ...
  if (updated) {
    await delCached(`business:getMine:${ctx.businessId}`);
    invalidateStatsCacheByPrefix(`business:setupStatus:${ctx.businessId}`);
  }

...
export async function updateBusinessBookingConfig(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
  ...
  const user = await db.select().from(users).where(eq(users.firebaseUid, ctx.firebaseUid as string)).then(r => r[0]);
  if (!user) {
...
export async function updateBusinessTimezone(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, timezone: string) {
...
export async function updateBusinessOrderSettings(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
...
export async function updateBusinessCustomizationSettings(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
  businessName?: string;
  logoBlobPath?: string;
...
export async function ensureBusinessWebsiteWidget(ctx: { businessId: string; userId: string; firebaseUid?: string | null }) {
  const [biz] = await db.select().from(businesses).where(eq(businesses.id, ctx.businessId)).limit(1);
  if (!biz) {
    throw new TRPCError({ code: "NOT_FOUND", message: "Business not found" });
  }
...
export async function disconnectBusinessGmailConnection(ctx: { businessId: string; userId: string; firebaseUid?: string | null }) {
  const now = new Date();
  const [updated] = await db
    .update(businesses)
    .set({
...
export async function completeBusinessOnboardingSetup(ctx: { businessId: string; userId: string; userEmail?: string | null; firebaseUid?: string | null }, input: {
  businessName: string;
  website?: string;
  phone?: string;
  address?: string;
...
export function filterSubscriptionRecord<T>(record: Record<string, T>, prefix: string): Record<string, T> {

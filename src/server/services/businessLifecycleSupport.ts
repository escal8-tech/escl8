...
import { upsertBusinessTimezone, upsertBusinessOrderSettings, upsertBusinessCustomizationSettings, upsertBusinessWebsiteWidgetSettings } from "@/server/services/businessSettingsStore";
...
import { delCached, REDIS_KEYS } from "@/lib/redis";
import {
...
async function invalidateBusinessCache(businessId: string) {
  await Promise.all([
    delCached(`${REDIS_KEYS.BUSINESS_MINE}${businessId}`),
    delCached(`${REDIS_KEYS.BUSINESS_SUB}${businessId}`),
    delCached(`${REDIS_KEYS.BUSINESS_PHONES}${businessId}`),
  ]);
}

export async function setWhatsappIdentityAutoReplyPaused(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: { phoneNumberId: string; autoReplyPaused: boolean }) {
  ...
  await invalidateBusinessCache(ctx.businessId);
...
export async function setWhatsappIdentityAiDisabled(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: { phoneNumberId: string; aiDisabled: boolean }) {
  const details = await db
    .select({
      channelIdentityId: whatsappIdentityDetails.channelIdentityId,
      displayPhoneNumber: whatsappIdentityDetails.displayPhoneNumber,
...
export async function ensureWebsiteWidget(ctx: { businessId: string; userId: string; firebaseUid?: string | null }) {
  const [biz] = await db.select().from(businesses).where(eq(businesses.id, ctx.businessId)).limit(1);
  if (!biz) {
    throw new TRPCError({ code: "NOT_FOUND", message: "Business not found" });
  }
...
export async function completeOnboardingSetup(ctx: { businessId: string; userEmail?: string | null; userId: string; firebaseUid?: string | null }, input: {
  businessName: string;
  website?: string;
  phone?: string;
  address?: string;

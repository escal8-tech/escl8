...
const lastMessageDirectionSelection = sql<string | null>`${messageThreads.lastMessageDirection}`;
...
export async function listRecentThreadsForBusiness(businessId: string, input: {
  limit: number;
  channelIdentityId?: string | null;
}) {
  const cacheKey = `messages:listRecentThreads:${businessId}:${JSON.stringify(input)}`;
...
export async function listRecentThreadsPageForBusiness(businessId: string, input: {
  limit: number;
  channelIdentityId?: string | null;
  query?: string;
  ...
  const cacheKey = `messages:listRecentThreadsPage:${businessId}:${JSON.stringify(input)}`;
  return withCache(cacheKey, 60, async () => {
  const whereConditions = [
    eq(messageThreads.businessId, businessId),
    isNull(messageThreads.deletedAt),
...
export async function searchCustomersForBusiness(businessId: string, input: {
  query: string;
  source?: string;
  limit: number;
}) {
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(customers.businessId, businessId),
        input.source ? eq(customers.source, input.source) : undefined,
        isNull(customers.deletedAt),
        or(
          ilike(customers.externalId, pattern),
          ilike(customers.phone, pattern),
...
export async function listThreadsForCustomerInBusiness(businessId: string, customerId: string) {
  const rows = await db
    .select({
...
export async function listMessagesForThread(businessId: string, input: {
  threadId: string;
  limit: number;
  cursor?: string;
}) {
...
export async function getOrderThreadAnchorForBusiness(businessId: string, orderId: string) {
...
export async function listThreadWindowForBusiness(businessId: string, input: {
  threadId: string;
...
export async function getThreadSessionWindowForBusiness(businessId: string, threadId: string) {
  const [thread] = await db
    .select({
      id: messageThreads.id,
      channelIdentityId: messageThreads.channelIdentityId,

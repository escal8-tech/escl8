...
export async function listRecentThreads(ctx: { businessId: string }, input: {
  limit: number;
  channelIdentityId?: string | null;
}) {
  const cacheKey = `messages:recent_threads:${ctx.businessId}:${input.channelIdentityId || "all"}:${input.limit}`;
...
export async function listRecentThreadsPage(ctx: { businessId: string }, input: {
  ...
  const cacheKey = `messages:recent_threads_page:${ctx.businessId}:${input.channelIdentityId || "all"}:${input.limit}:${input.query || ""}:${input.cursorThreadId || ""}`;

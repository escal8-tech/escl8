...
export async function listRecentThreads(businessId: string, input: {
  limit?: number;
...
export async function listRecentThreadsPage(businessId: string, input: {
  limit?: number;
  channelIdentityId?: string | null;
  query?: string;
...
export async function searchCustomers(businessId: string, input: {
  query: string;
  source?: string;
  limit?: number;
}) {
...
export async function listThreadsForCustomer(businessId: string, customerId: string) {
  const rows = await db
    .select({
...
export async function listMessages(businessId: string, input: {
  threadId: string;
  limit?: number;
  cursor?: string;
}) {
...
export async function getOrderThreadAnchor(businessId: string, orderId: string) {
  ...
  const [legacyAnchor] = await db
    ...
    .where(
      and(
        eq(threadMessages.threadId, order.threadId),
        or(
          sql`${threadMessages.meta} -> 'orderAnchor' ->> 'orderId' = ${orderId}`,
          sql`${threadMessages.meta} -> 'sourceMeta' ->> 'orderId' = ${orderId}`,
        ),
      ),
    )
    .orderBy(desc(threadMessages.createdAt), desc(threadMessages.id))
...
export async function listThreadWindow(businessId: string, input: {
  threadId: string;
...
export async function getThreadSessionWindow(businessId: string, threadId: string) {
  const [thread] = await db
    .select({
      id: messageThreads.id,
      channelIdentityId: messageThreads.channelIdentityId,

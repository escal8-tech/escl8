...
export async function listRecentThreads(ctx: Context, input: { limit: number; channelIdentityId?: string | null }) {
  const cacheKey = `threads:recent:${ctx.businessId}:${input.channelIdentityId || "all"}:${input.limit}`;
...
export async function listRecentThreadsPage(ctx: Context, input: {
  limit: number;
...
export async function searchCustomers(ctx: Context, input: {
  query: string;
  source?: string;
  limit?: number;
}) {
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(customers.businessId, ctx.businessId),
        eq(customers.source, input.source ?? "whatsapp"),
        isNull(customers.deletedAt),
        or(
          ilike(customers.externalId, pattern),
          ilike(customers.phone, pattern),
...
export async function listThreadsForCustomer(ctx: Context, input: { customerId: string }) {
  const rows = await db
    .select({
...
export async function listMessages(ctx: Context, input: {
  threadId: string;
  limit?: number;
  cursor?: string;
}) {
...
export async function getOrderThreadAnchor(ctx: Context, input: { orderId: string }) {
...
export async function listThreadWindow(ctx: Context, input: {
  threadId: string;
  ...
  const beforeLimit = input.beforeLimit ?? 24;
  const afterLimit = input.afterLimit ?? 6;
  ...
  if (input.olderCursor) {
    ...
    const hasMoreBefore = rows.length > beforeLimit;
    const batch = hasMoreBefore ? rows.slice(0, beforeLimit) : rows;
    return {
  ...
  if (input.newerCursor) {
    ...
    const hasMoreAfter = rows.length > afterLimit;
    const batch = hasMoreAfter ? rows.slice(0, afterLimit) : rows;
    return {
      messages: batch,
      anchorMessageId: input.anchorMessageId ?? null,
      hasMoreBefore: false,
  ...
  const beforeBatch = hasMoreBefore ? beforeRows.slice(0, beforeLimit) : beforeRows;
  const hasMoreAfter = afterRows.length > afterLimit;
  const afterBatch = hasMoreAfter ? afterRows.slice(0, afterLimit) : afterRows;
...
export async function getThreadSessionWindow(ctx: Context, input: { threadId: string }) {
  const [thread] = await db
    .select({
      id: messageThreads.id,
      channelIdentityId: messageThreads.channelIdentityId,

...
export async function listRecentThreads(ctx: { businessId: string }, input: { limit: number; channelIdentityId?: string | null }) {
  const cacheKey = `messages:recentThreads:${ctx.businessId}:${input.channelIdentityId || "all"}:${input.limit}`;
...
export async function listMessages(ctx: { businessId: string }, input: { threadId: string; limit: number; cursor?: string }) {
  const cacheKey = `messages:list:${input.threadId}:${input.limit}:${input.cursor || "none"}`;
  return withStatsCache(cacheKey, 60, async () => {
...
export async function listThreadWindow(ctx: { businessId: string }, input: { threadId: string; anchorMessageId?: string; beforeLimit: number; afterLimit: number; olderCursor?: string; newerCursor?: string }) {
  const [thread] = await db
    .select({ id: messageThreads.id })
    .from(messageThreads)
    .where(

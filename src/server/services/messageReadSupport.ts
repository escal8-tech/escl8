...
import { withCache, REDIS_KEYS } from "@/lib/redis";
...
export async function listRecentThreads(ctx: { businessId: string }, input: { limit: number; channelIdentityId?: string | null }) {
  const cacheKey = `${REDIS_KEYS.MESSAGE_THREADS}${ctx.businessId}:list:${input.channelIdentityId || "all"}:${input.limit}`;

  return withCache(cacheKey, REDIS_KEYS.GENERIC_TTL, async () => {
    ...
    const rows = await db
      .select({
        ...
        // Optimized to use denormalized column with a simple coalesce fallback
        lastMessageDirection: sql<string | null>`coalesce(${messageThreads.lastMessageDirection}, 'inbound')`,
...
export async function searchCustomers(ctx: { businessId: string }, input: { query: string; source?: string; limit: number }) {

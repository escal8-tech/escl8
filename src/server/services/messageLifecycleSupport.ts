...
export async function sendTextForBusiness(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
  threadId: string;
  text: string;
}) {
  ...
  await scanDelCached(`messages:listRecentThreadsPage:${ctx.businessId}`);
...
export async function sendMediaForBusiness(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
  threadId: string;
  messages: any[];
}) {
  const [thread] = await db

...
import { delCached, getCached, setCached } from "@/lib/redis";
...
export async function getOrderThreadAnchor(
  ctx: MessageContext,
...
export async function listRecentThreads(
  ...
  const cacheKey = `messages:threads:recent:${ctx.businessId}:${input.channelIdentityId || "all"}:${input.limit}`;
...
export async function invalidateRecentThreadsCache(businessId: string) {
  // We don't know the exact limit or channel identity used in the cache key easily here,
  // but we can either use a pattern delete if redis supports it, or just accept that
  // the most common UI view (default limit, all channels) is what needs invalidation.
  // Given our setCached doesn't expose pattern delete, and we want to be safe:
  const commonKeys = [
    `messages:threads:recent:${businessId}:all:50`,
    `messages:threads:recent:${businessId}:all:20`,
    `messages:threads:recent:${businessId}:all:100`,
  ];
  await Promise.all(commonKeys.map(key => delCached(key)));
}

export async function listRecentThreadsPage(
  ctx: MessageContext,
  input: {
    limit: number;
    channelIdentityId?: string | null;
...
export async function getThreadSessionWindow(ctx: MessageContext, input: { threadId: string }) {
  const [thread] = await db
    .select({
      id: messageThreads.id,
      channelIdentityId: messageThreads.channelIdentityId,
...
export async function sendText(ctx: MessageContext, input: { threadId: string; text: string }) {
  const [thread] = await db
    .select({
      id: messageThreads.id,
      channelIdentityId: messageThreads.channelIdentityId,
  ...
  await invalidateRecentThreadsCache(ctx.businessId);

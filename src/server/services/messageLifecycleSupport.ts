...
import { scanDelCached, REDIS_KEYS } from "@/lib/redis";
...
async function invalidateThreadCache(businessId: string) {
  // Invalidate all thread list variations for this business
  await scanDelCached(`${REDIS_KEYS.MESSAGE_THREADS}${businessId}:*`);
}
...
export async function sendText(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: { threadId: string; text: string }) {
  ...
  await invalidateThreadCache(ctx.businessId);
...
export async function sendMedia(ctx: { businessId: string; userId: string; firebaseUid?: string | null }, input: {
  threadId: string;
  messages: any[]; // Kept as any[] for now to avoid re-defining schema here, though not ideal
}) {
  const [thread] = await db
    .select({
      id: messageThreads.id,

...
import { invalidateStatsCacheByPrefix } from "../lib/statsCache";
...
export async function sendText(ctx: Context, input: { threadId: string; text: string }) {
  ...
  invalidateStatsCacheByPrefix(`threads:recent:${ctx.businessId}:`);
...
export async function sendMedia(ctx: Context, input: { threadId: string; messages: MediaPart[] }) {
  const [thread] = await db
    .select({
      id: messageThreads.id,
      channelIdentityId: messageThreads.channelIdentityId,

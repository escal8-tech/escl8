...
export async function sendText(businessId: string, userId: string | null, firebaseUid: string | null, input: {
  threadId: string;
  text: string;
}) {
...
export async function sendMedia(businessId: string, userId: string | null, firebaseUid: string | null, input: {
  threadId: string;
  messages: MediaPart[];
}) {
  const [thread] = await db

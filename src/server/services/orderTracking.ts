...
export function createOrderTrackingToken(input: { businessId: string; orderId: string }): string {
  // Ensure components do not contain colons to preserve parsing logic
  const bid = input.businessId.replace(/:/g, "_");
  const oid = input.orderId.replace(/:/g, "_");
  const combined = `${bid}:${oid}`;
  const payloadBase = Buffer.from(combined).toString("base64url");
...
export function parseOrderTrackingToken(token: string): ParsedTrackingToken | null {
  ...
  if (compactMatch) {
    ...
    const combined = cleanText(Buffer.from(compactMatch[1], "base64url").toString("utf8"), 320);
    const [businessId, ...rest] = combined.split(":");
...
export async function getPublicOrderTrackingData(token: string): Promise<PublicOrderTrackingData | null> {
  ...
  const orderPredicate = and(eq(orders.id, parsed.orderId), eq(orders.businessId, parsed.businessId));

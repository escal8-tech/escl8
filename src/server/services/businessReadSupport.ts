...
export async function getMine(ctx: { businessId: string; userEmail?: string | null }, input: { email?: string }) {
  if (ctx.userEmail && input.email && input.email !== ctx.userEmail) {
    throw new TRPCError({ code: "FORBIDDEN", message: "Email mismatch" });
  }

  const cacheKey = `biz:${ctx.businessId}:getMine`;
...
export async function getCustomizationPreview(ctx: { businessId: string }) {
...
export async function getSubscription(ctx: { businessId: string }) {
  const cacheKey = `biz:${ctx.businessId}:subscription`;
  return withStatsCache(cacheKey, 60, async () => {
    const [biz] = await db
      .select({

...
export async function listPhoneNumbersForBusiness(businessId: string) {
  const cacheKey = `business:listPhoneNumbers:${businessId}`;
...
export async function getMineForBusiness(ctx: { businessId: string; userEmail?: string | null }, inputEmail?: string) {
  const cacheKey = `business:getMine:${ctx.businessId}`;
  return withCache(cacheKey, 60, async () => {
  if (ctx.userEmail && inputEmail && inputEmail !== ctx.userEmail) {
...
export async function getCustomizationPreviewForBusiness(businessId: string) {
...
export async function getSubscriptionForBusiness(businessId: string) {
  const cacheKey = `business:getSubscription:${businessId}`;
